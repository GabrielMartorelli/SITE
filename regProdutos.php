...
<html>
  ...
  <body>
    <form method="post">
	      ...
	      <a href="altProdutos.php" id="a-logar">Alterar</a>
        <a href="logar.php" id="a-sair">Sair</a>
	      <input type="submit" value="REGISTRAR" class="btn-submit-reg"/>
			</div>
    </form>
    <?php
    if (isset($_POST['codigo'])) {//verifica se algum campo não foi preenchido
      ...
      //addslashes é tipo de tratramento de string para deixar o sistema menos vulnerável
	      ...
	      if ($u->msgErro == "") {//tratamendo de erro de conexão com banco de dados
	        if ($u->regProduto($codigo, $produto, $quantidade, $preco)) {//verifica de o email e senha consta no banco de dados
						?>
	          <lable class="msg-sucesso">cadastro de produto feito com sucesso</label>
						<?php
	        }else {//se não consta o email ou senha no banco de dados é mostrado esse ereo
						?>
	          <label class="msg-erro-reg">Produto já cadastrado</lable>
						<?php
	        }
